...
jest.mock("@pankod/refine-antd", () => {
  ...
  const Form: any = ({ children, onFinish, ...rest }: any) => {
    if (children && React.Children.count(children) > 0) {
      return (
        <form
          data-testid="client-hardware-create-form"
          {...rest}
          onSubmit={(e: any) => {
            e.preventDefault();
            try {
      ...
      <div>
        <select
          ...
          onChange={(e) => {
            ...
            setVal(v);
            const label = e.target.options[e.target.selectedIndex]?.text ?? v;
            onChange?.(v);
            try {
        ...
        {placeholder === "hardware.label.placeholder.status" && val === "2" && (
          <select data-testid="select-hardware.label.placeholder.user">
            <option value="">--</option>
...
import { ClientHardwareCreate } from "pages/hardware_client/create";
...
describe("ClientHardwareCreate - Check render & Basic workflows", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("renders form fields and buttons", () => {
    renderComponent();
    expect(
      screen.getByTestId("client-hardware-create-form")
    ).toBeInTheDocument();
    expect(
