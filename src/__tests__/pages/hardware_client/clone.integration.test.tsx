...
jest.mock("@pankod/refine-antd", () => {
  ...
  const Form: any = ({ children, onFinish, ...rest }: any) => {
    if (children && React.Children.count(children) > 0) {
      return (
        <form
          data-testid="hardware-clone-form-client"
          {...rest}
          onSubmit={(e: any) => {
            e.preventDefault();
            try {
...
import { ClientHardwareClone } from "pages/hardware_client/clone";

describe("ClientHardwareClone - import real component with mocked deps", () => {
