  ...
  return (
    ...
    >
      <Tabs defaultActiveKey={defaultValue.active}>
        <TabPane
          tab={t("user.label.field.information")}
          key={defaultValue.active}
        >
          <Row gutter={16}>
        ...
        <TabPane
          tab={t("user.label.field.permission")}
          key={defaultValue.inactive}
        >
          <div className="title_permission">
            <Form.Item name="permissions" initialValue={data?.permissions}>
              <div>
                ...
                {!isFetching &&
                  Object.values(Permission).map((key, index) => (
                    <div className="list-permission" key={index}>
                      <Row
                        gutter={{
                          xs: 8,
                          sm: 16,
                        ...
                        {index < 5 && (
                          <Col className="gutter-row" span={17}>
                            <Form.Item
                              name={`${key.name}`}
                              initialValue={
                                data?.permissions[key.name]?.toString() ?? "0"
                              }
                            >
                              <Radio.Group
                                ...
                                onChange={(event) => {
                                  ...
                                  if (
                                    event.target.value === AccessType.allow &&
                                    key.name === Permission.branchadmin.name
                                  ) {
                                    setShowCheckboxList(true);
                                  }
                                  if (
                                ...
                                defaultValue={
                                  permissionOfUser[key.name]?.toString() ?? "0"
                                }
                      ...
                      {showCheckboxList && index === 2 && (
                        <div className="list-permission">
                          ...
                          >
                            <Col className="gutter-row" span={6}>
                              <div>
                                {t(`user.label.title.choose_branchadmin`)}
                              </div>
                            </Col>
                            ...
                            >
                              <Col className="gutter-row" span={6}>
                                {item.code === "view.requestable" ? (
                                  <div>
                                    {t(`user.label.title.view_requestable`)}
                                  </div>
                                ) : (
                                  <div>
                                    {t(`user.label.title.${item.code}`)}
                                  </div>
                                )}
                              </Col>
                              <Col className="gutter-row" span={17}>
                                <Form.Item
                                  name={`${key.name}.${item.code}`}
                                  initialValue={
                                    data?.permissions[
                                      `${key.name}.${item.code}`
                                    ]?.toString() ?? "0"
