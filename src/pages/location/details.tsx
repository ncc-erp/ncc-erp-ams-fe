import { Typography, Tabs } from "@pankod/refine-antd";
...
export const LocationDetails: React.FC<IResourceComponentsProps> = () => {
  ...
  return (
    <>
      <Title level={3}>
        {" "}
        {translate("location.label.title.nameTitle")}: {location_name}
      </Title>
      <Tabs defaultActiveKey={defaultValue.active}>
        <TabPane
          tab={translate("location.label.title.assets")}
          key={defaultValue.active}
        >
          <DetailsAsset id_name="rtd_location_id" />
        </TabPane>
        <TabPane
          tab={translate("location.label.title.tools")}
