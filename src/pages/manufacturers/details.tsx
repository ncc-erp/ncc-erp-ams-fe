...
export const ManufacturesDetails: React.FC<IResourceComponentsProps> = () => {
  ...
  return (
    <>
      <Title level={3}>
        {translate("manufactures.label.title.nameTitle")}: {manufactures_name}
      </Title>
      <Tabs defaultActiveKey={defaultValue.active}>
        <TabPane
          tab={translate("manufactures.label.title.asset")}
          key={defaultValue.active}
        >
          <DetailsAsset id_name="manufacturer_id" />
        </TabPane>
        <TabPane
          tab={translate("manufactures.label.title.accessories")}
          key={defaultValue.inactive}
        >
          <DetailsAccessory id_name="manufacturer_id" />
        </TabPane>
        <TabPane
          tab={translate("manufactures.label.title.consumables")}
          key="3"
        >
